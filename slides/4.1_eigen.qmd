...
:::
::: {.column width="45%"}
![10-graphing-polynomials-01.png](../assets/4.0_linalg/10-graphing-polynomials-01.png){width="100%" fig-alt="Graph of a polynomial function f(x) with two local maxima (labeled (-3,2) and (2,1)) and two local minima (labeled (-1,-3) and (4,-1)) marked with red dots."}
:::
::::
...
![Screenshot 2025-11-07 at 12.20.38 PM.png](../assets/4.0_linalg/Screenshot-2025-11-07-at-12.20.38-PM.png){width="70%" fig-alt="Equation (11.3.7): a block-matrix identity showing the matrix P1 (containing a smaller (n-1)-dimensional sub-block (n-1)P1) multiplied by matrix A, equal to a reduced matrix with pivot entries a00 and k in the first column and an 'irrelevant' block filling the remainder."}

![Screenshot 2025-11-07 at 12.20.59 PM.png](../assets/4.0_linalg/Screenshot-2025-11-07-at-12.20.59-PM.png){width="70%" fig-alt="Equation A' = P.A.P: a matrix with pivot entries a00 and k arranged along the top-left border and an 'irrelevant' block filling the remainder, resulting from conjugating A by the permutation/reflection matrix P."}
...
![](../assets/4.0_linalg/slide65_Screenshot-2025-11-07-at-12.20.59-PM.png){fig-alt="Equation A' = P.A.P: a matrix with pivot entries a00 and k arranged along the top-left border and an 'irrelevant' block filling the remainder, resulting from conjugating A by the permutation/reflection matrix P."}

![](../assets/4.0_linalg/slide65_Screenshot-2025-11-07-at-12.21.16-PM.png){fig-alt="Definition of P2: a block matrix with a 2x2 identity block in the upper-left corner and a smaller (n-2)-dimensional sub-block (n-2)P2 filling the rest."}

- Next iteration:

![](../assets/4.0_linalg/slide65_Screenshot-2025-11-07-at-12.20.38-PM.png){fig-alt="Equation (11.3.7): a block-matrix identity showing the matrix P1 (containing a smaller (n-1)-dimensional sub-block (n-1)P1) multiplied by matrix A, equal to a reduced matrix with pivot entries a00 and k in the first column and an 'irrelevant' block filling the remainder."}
...
![](../assets/4.0_linalg/slide66_Screenshot-2025-11-07-at-12.17.34-PM.png){fig-alt="Vector diagram illustrating a Householder reflection: vector x and its reflection ||x||y drawn from a common origin symmetric about a dashed horizontal axis, with the difference vector x - ||x||y drawn perpendicular to that axis."}
...
![Screenshot 2025-11-09 at 6.22.46 PM.png](../assets/4.0_linalg/Screenshot-2025-11-09-at-6.22.46-PM.png){width="70%" fig-alt="Diagram of a matrix sparsity pattern showing an upper-Hessenberg-like structure: X marks fill the upper triangle and the first subdiagonal, with blanks (implied zeros) below."}
...
![](../assets/4.0_linalg/slide78_fig-11.4.1a.png){fig-alt="Diagram of a linear triatomic molecule model: three masses (m, M, m) connected by two springs (K, K) along the x-axis, with displacements x1, x2, x3 labeled; three rows (a), (b), (c) below show different relative motion patterns (all moving together, one mass fixed, and a symmetric stretch) with arrows indicating each mass's direction of motion."}
...
![](../assets/4.0_linalg/slide79_fig-11.4.1a.png){fig-alt="Diagram of a linear triatomic molecule model: three masses (m, M, m) connected by two springs (K, K) along the x-axis, with displacements x1, x2, x3 labeled; three rows (a), (b), (c) below show different relative motion patterns (all moving together, one mass fixed, and a symmetric stretch) with arrows indicating each mass's direction of motion."}
